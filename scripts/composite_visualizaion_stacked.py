...
def main():
    ...
    # ---------------------------
    # Assume the composite scores for each metric are stored in columns named:
    # 'avg_ks_stat_score', 'avg_chi2_score', 'spearman_diff_score', 'cramers_v_diff_score', and 'mi_diff_score'
    metrics = ['avg_ks_stat_score', 'avg_chi2_score', 'spearman_diff_score', 'cramers_v_diff_score', 'mi_diff_score']

    top10 = df_ranked.head(10)
    plt.figure(figsize=(12, 8))
    bottom = np.zeros(len(top10))
    for metric in metrics:
        plt.bar(top10['dataset'], top10[metric], bottom=bottom, label=metric)
        bottom += top10[metric].values

    plt.xticks(rotation=45)
    plt.xlabel("Synthetic Dataset")
    plt.ylabel("Composite Score (Higher = Better)")
    plt.title("Stacked Composite Scores (Top 10 Synthetic Models)")
    plt.legend()
    plt.tight_layout()
    stacked_plot_path = os.path.join(OUTPUT_DIR, "stacked_composite_scores_top10.png")
    plt.savefig(stacked_plot_path, dpi=600)
    plt.close()
    print("Stacked composite ranking plot saved to:", stacked_plot_path)
