...
def main():
    ...
    # -----------------------------
    # Visualization: Heatmap for Top 10 Models by Correlation Metrics
    # -----------------------------
    # Select top 10 models based on one of the metrics (e.g., Spearman Diff)
    top10 = df_results.sort_values("spearman_diff", ascending=True).head(10)

    # Set the index to the dataset for a better heatmap layout (rows = models, columns = metrics)
    top10 = top10.set_index("dataset")

    plt.figure(figsize=(10, 6))
    # Plot the heatmap showing all three metrics; annot=True displays values on the cells.
    sns.heatmap(top10[['spearman_diff', 'cramers_v_diff', 'mi_diff']],
                annot=True, cmap="YlGnBu", fmt=".3f")
    plt.title("Heatmap of Correlation Metrics Differences for Top 10 Models")
    plt.xlabel("Metric")
    plt.ylabel("Synthetic Dataset")
    plt.tight_layout()

    heatmap_path = os.path.join(OUTPUT_DIR, "heatmap_top10_metrics.png")
